...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <FiShoppingCart size={20} color={cartColor} />
      </button>
      ...
      <Link to={`/product/${id}`} style={{ textDecoration: "none", color: "inherit" }}>
        ...
        <div style={{ padding: "16px 16px 18px 16px", display: "flex", flexDirection: "column", flex: 1 }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: "#222", marginBottom: 4 }}>
            ...
            {hasDiscount && (
              <>
                <span style={{ fontSize: 13, color: "#888", fontWeight: 400, marginLeft: 8, textDecoration: "line-through" }}>
                  ₹{originalPrice}
                </span>
                <span style={{ fontSize: 12, color: "#388e3c", fontWeight: 600, marginLeft: 6 }}>
                  {discount}% OFF
                </span>
              </>
            )}
